...
class Fee extends \Magento\Quote\Model\Quote\Address\Total\AbstractTotal
{
  ...
  public function collect(
	...
	public function getFinalFeeAmount(\Magento\Quote\Model\Quote $quote)
	{
		...
		$powercoatingItemsColorCode = $this->checkoutSession->getIsSetPowderCoatingColor();
		$powercoatingItems = [];		
		...
		foreach($items as $item){
			...
			if($_product->getPowder_coating_available()){
				...
				$logger->info("Product Fee");  
				$logger->info($item->getSku());  
				$logger->info($finalPwdCtPrice);  
		...
		if($isPowederCoatingCart == 1){
			...
			if($overall_price_for_powdder_coating < 70 ){ 			
				...
				if(in_array($powercoatingItemsColorCode,$minimumPowderCoatingFeeColor0)){
					...
					if($overall_price_for_powdder_coating < 50 ) {
