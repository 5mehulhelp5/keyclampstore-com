...
class FormulaPricePlugin
{
    ...
    public function aroundGetFinalPrice(
        ...
        !empty($product->getFormulaPriceEnable())) {
        ...
        $finalPrice = $proceed($qty, $product);
        $result = $this->localeFormat->getNumber($finalPrice, true);
        return $result;
    }
                                    ...
                                    if ($option->getType() == 'drop_down' ||
                                        ...
                                        if ($option->getValues()) {
                                            foreach ($option->getValues() as $value) {
                                                ...
                                                if ($oId == $rowvalue) {
                                                    $selectTitle =
                                                    "{" . strtolower(trim($value->getDefaultTitle())) . "}";
                    ...
                    foreach ($extraPriceFormula as $ekey => $eval) {
                        ...
                        if (!preg_match('/[a-z]+/', $ekey) || !preg_match('/[a-z]+/', $eval)) {
                            ...
                            if ($ekey == 1) {
                                ...
                                $finalPrice = $finalPrice ? number_format($finalPrice, 2) : 0;
                            }
                        } else {
                            foreach ($optionValues as $key => $val) {
    ...
    public function calculateString($mathString)
    {
        ...
        $compute = $this->stringcalc->calculate($mathString);
        return 0 + $compute;
    }
}
