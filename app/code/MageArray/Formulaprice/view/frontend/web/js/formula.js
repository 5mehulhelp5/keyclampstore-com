	define([
		...
		function getFinalPrice(alloptions, equation, labels, minCheck, maxCheck, maxMessage, minMessage, extraPriceFormula, getQty, addQty, custValue, proPrice) {
            ...
            jQuery.validator.addMethod("validate-number-col", function (currValue, element) {
                var currId = jQuery(element).attr('id');
                if(!jQuery.isNumeric(currValue)){
                    return false;
                }
                return true;
            }, function () {
                return jQuery.mage.__("Please enter numeric value");
            });
